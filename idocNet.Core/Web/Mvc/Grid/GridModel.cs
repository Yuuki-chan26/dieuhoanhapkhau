...
namespace idocNet.Core.Web.Mvc.Grid
{
    ...
    {
        public GridModel(Type modelType, IEnumerable dataSource)
        {
            this.ModelMetadata = ModelMetadataProviders.Current.GetMetadataForType(() => null, modelType);

            DataSource = dataSource;

            Initialize(ModelMetadata, dataSource);
        }
        ...
        protected virtual void Initialize(ModelMetadata modelMetadata, IEnumerable dataSource)
        {
            if (ModelMetadata is NextPortalModelMetadata)
            {
                var gridAttribute = ((NextPortalModelMetadata)modelMetadata).Attributes.OfType<GridAttribute>().FirstOrDefault();
            ...
            PageSizeDifinition = ((NextPortalModelMetadata)modelMetadata).Attributes.OfType<PageSizeAttribute>().FirstOrDefault();

            GridColumns = GetColumns(ModelMetadata);
            GridActions = GetActions(ModelMetadata);
            GridItems = GetItems(this.DataSource, GridColumns, GridActions);
            GridCommandDefinitions = GetGridCommandDefinitions(ModelMetadata);
        }
        protected virtual IEnumerable<GridAction> GetActions(ModelMetadata modelMetadata)
        {
            List<GridAction> actions = new List<GridAction>();
            if (modelMetadata is NextPortalModelMetadata)
            {
                foreach (var item in ((NextPortalModelMetadata)modelMetadata).Attributes.OfType<GridActionAttribute>().OrderBy(it => it.Order))
                {
                    ...
                    if (item.VisibleArbiter != null)
                    {
                        gridAction.VisibleArbiter = (IVisibleArbiter)Activator.CreateInstance(item.VisibleArbiter);
                    }
        ...
        protected virtual IEnumerable<GridColumn> GetColumns(ModelMetadata modelMetadata)
        {
        ...
        protected virtual IEnumerable<GridColumn> GetGridColumns(ModelMetadata propertyMetadata)
        {
            if (ModelMetadata is NextPortalModelMetadata)
            {
                foreach (var att in ((NextPortalModelMetadata)propertyMetadata).Attributes.OfType<GridColumnAttribute>())
                {
        ...
        protected virtual IEnumerable<GridCommandAttribute> GetGridCommandDefinitions(ModelMetadata modelMetadata)
        {
            if (modelMetadata is NextPortalModelMetadata)
            {
                return ((NextPortalModelMetadata)modelMetadata).Attributes.OfType<GridCommandAttribute>().OrderBy(it => it.Order).ToArray();
            }
            else
            {
