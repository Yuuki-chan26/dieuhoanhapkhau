...
namespace idocNet.Core.Web.Mvc.Html
{
    public static class ValidationExtensions
    {
        ...
        private static string GetInvalidPropertyValueResource(HttpContextBase httpContext)
        {
            ...
            return (str ?? SR.System_Web_Mvc_Resources.GetString("Common_ValueNotValidForProperty"));
        }
        ...
        public static IHtmlString ValidationMessageForInput(this HtmlHelper htmlHelper, string inputName, IDictionary<string, object> htmlAttributes = null)
        {
            var validationMessage = "";
            string fullHtmlFieldName = inputName;
            if (!htmlHelper.ViewContext.UnobtrusiveJavaScriptEnabled)
            {
                throw new Exception("idocNet.Core.Web.Mvc.Html.ValidationExtensions.ValidationMessageForInput only support UnobtrusiveJavaScriptEnabled.");
            }
            ModelState modelState = htmlHelper.ViewData.ModelState[fullHtmlFieldName];
            ModelErrorCollection errors = (modelState == null) ? null : modelState.Errors;
            ModelError error = ((errors == null) || (errors.Count == 0)) ? null : errors[0];
