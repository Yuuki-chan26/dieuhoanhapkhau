...
namespace idocNet.Core.Web.Mvc.Paging
{
    public class PagerBuilder
    {
        ...
        public PagerBuilder(HtmlHelper helper, string actionName, string controllerName, int totalPageCount,
        ...
        protected virtual string GenerateUrl(int pageIndex)
        {
			...
			{
				prefix=string.Format("/{0}", SitePath);
			}
            if (!string.IsNullOrEmpty(_routeName))
                return  prefix+ urlHelper.RouteUrl(_routeName, routeValues);
			return prefix + urlHelper.RouteUrl(routeValues);
        }
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? _html.Encode(item.Text)
												 : (_pagerOptions.UseAjaxLink ? String.Format("<a href='{0}' class='ajax-link'>{1}</a>", url, item.Text) : String.Format("<a href='{0}'>{1}</a>", url, item.Text)));
        }
