...
function SetNodeValue(node, value) {
   ...
   function isImage(filename) {
    ...
    return str.indexOf(suffix, str.length - suffix.length) !== -1;
   	...
   	var imageTypes = ['.jpg', '.jpeg', '.bmp', '.png', '.gif'];

   	filename = filename.toLowerCase();
   	var ret = false;

   	$(imageTypes).each(function (index, item) {
   		if (endsWith(filename, item)) {
   			ret = true;
   		}
   	});
...
//					imagePath='/Scripts/tinymce/plugins/netadvimage/img/folder-horizontal.gif';
//        			}




        			$('#img-list').append('<li><a href="#" title="' + image.FileName + '"><span><img src="' + imagePath + '" alt="'+image.FileName+'" /></span>' + image.FileName + '</a><input type="hidden" value="' + image.FileName + '" /></li>');
        		});
        	}
        	else
        		ShowOverlays();
...
var ImageDialog = {
	...
	init: function (ed) {
		...
		var f = document.forms['mce'],
                ...
                uploader = new qq.FileUploader({
                	...
                	action: '/Scripts/tinymce/plugins/netadvimage/index',
                	params: {
                		path: null
                	},
	...
	pickup: function () {
		...
		tinyMCE.execCommand("mceSetContent", false, f.src.value);
	...
	insertAndClose: function () {
		...
		if (el && el.nodeName == 'IMG') {
			...
			if (isImage(nl.src.value)) {
				...
				ed.execCommand('mceInsertContent', false, '<a  href="' + nl.src.value + '">' + nl.src.value + '</a>', { skip_undo: 1 });
				ed.undoManager.add();
			}
		}
